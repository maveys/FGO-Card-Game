...
Board.prototype.addPlayer = function(username, socketId) {
...
Board.prototype.initGame = function() {
    ...
    for (var i = 0; i < 60; i++) {
        if (i % 2 === 0) {
            ...
            if (this.players[1]) this.players[1].deck.push(deck[i]);
        }
    } 
...
Board.prototype.getPlayer = function(socketId) {
    if (this.players[0] && this.players[0].socketId === socketId) return this.players[0];
    else if (this.players[1] && this.players[1].socketId === socketId) return this.players[1];
}

Board.prototype.getOpponentPlayer = function(socketId) {
    if (this.players[0] && this.players[0].socketId !== socketId) return this.players[0];
    else if (this.players[1] && this.players[1].socketId !== socketId) return this.players[1];
}

Board.prototype.getPlayerHand = function(socketId) {
    if (this.players[0] && this.players[0].socketId === socketId) return this.players[0].hand;
    else if (this.players[1] && this.players[1].socketId === socketId) return this.players[1].hand;
}
...
Board.prototype.removePlayer = function(socketId) {
    for (var i = 0; i < this.players.length; i++) {
        if (this.players[i].socketId === socketId) this.players.splice(i, 1);
    }
}
